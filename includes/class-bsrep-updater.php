 ...
 *
 * Ported from Bsm_Updater in bs-maintenance on September 22, 2026, kept as close to
 * that copy as the naming rules allow so a fix found in one can be carried to the
 * other by eye. Two deliberate differences: indentation is tabs, per the code
 * standards in THEME-RULES.md, and the identifiers carry this plugin's prefix.
 * Short array syntax is the source's and was left rather than rewritten, because a
 * mechanical [ ] to array() pass over 666 lines risks a real bug and buys nothing.
 *
...
class Bsrep_Updater {
	...
	const REPO = 'biscuitstudios/bs-site-report';

	const CACHE_KEY = 'bsrep_updater_release_v1';
	...
	private function fetch_release() {
		...
		if ( is_wp_error( $response ) ) {
			error_log( '[BSREP] Update check failed: ' . $response->get_error_message() );
		...
		if ( 200 !== $code ) {
			...
			error_log( '[BSREP] Update check failed: HTTP ' . $code . ' from ' . self::REPO );
		...
		if ( ! is_array( $body ) || [] === $body ) {
			error_log( '[BSREP] Update check failed: no releases listed for ' . self::REPO );
		...
		if ( [] === $log ) {
			error_log( '[BSREP] Update check failed: no usable release tags for ' . self::REPO );
		...
		if ( '' === $package ) {
			error_log( '[BSREP] Update check failed: release v' . $latest['version'] . ' has no zip asset.' );
	...
	private function render_changelog( $release ) {
		...
		foreach ( $release['log'] as $entry ) {
			...
			$out .= '' === $notes
				? '<p>' . esc_html__( 'No release notes were published for this version.', 'bs-site-report' ) . '</p>'
		...
		$out .= sprintf(
			...
			$release['truncated']
				? esc_html__( 'Earlier releases are on GitHub', 'bs-site-report' )
				: esc_html__( 'View all releases on GitHub', 'bs-site-report' )
		);
	 ...
	 * @param array $entry
	 */
	private static function heading( $entry ) {
	 ...
	 */
	private static function format_date( $published ) {
		if ( '' === $published ) {
			return '';
		}
	 ...
	 */
	private static function strip_compare_link( $notes ) {
	 ...
	 * @return array
	 */
	private function readme_changelog() {
